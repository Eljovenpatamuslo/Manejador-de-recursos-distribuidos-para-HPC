...
ClienteConexion *crear_cliente(int clienteFD, int tipo, const char ip[],
                               char *mensaje) {
    ClienteConexion *nuevoCliente = malloc(sizeof(ClienteConexion));
    if (!nuevoCliente) return NULL;
...
int leer_y_procesar_cliente(ClienteConexion *cliente, RecursosNodo recNodo,
    ...
    if (cliente->tipo == CONEXION_SALIENTE) {
        ...
        if (error != 0) {
            fprintf(stderr, "[WARN] Fallo al conectar con la IP %s. Razón: %s\n",
    ...
    int bytes_leidos = read(cliente->fd,
                            cliente->buffer + cliente->bytes_in_buffer,
                            espacio_libre);
    if (bytes_leidos == 0) {
        ...
        *newline_ptr = '\0';
        ...
        if (cliente->tipo == CLIENTE_AGENTE_C) {
            ...
            manejar_cliente_erlang(cliente, cliente->buffer, tablaJobs,
                                   tablaNodos, epollFd, recNodo, miIp, puertoTcp);
        }
...
int tipo_recurso_desde_string(char *s) {
    if (strcmp("cpu", s) == 0) return 0;
    if (strcmp("mem", s) == 0) return 1;
    if (strcmp("gpu", s) == 0) return 2;
    return -1;
}
...
void notificar_lista_promovidos(ListaPromovidos lista) {
    ...
    while (actual != NULL) {
        printf("[PROMOVIDOS] Notificando a FD %d (Job %lu)\n",
               actual->cliente->fd, actual->jobId);
        if (enviar_formateado(actual->cliente->fd, "GRANTED %lu\n", actual->jobId) == 0) {
...
void manejar_agente_c(ClienteConexion *cliente, const char *mensaje,
    ...
    if (strcmp(comando, "RESERVE") == 0) {
        if (sscanf(mensaje, "RESERVE %lu %15s %lu", &jobId, recurso, &cant) == 3) {
            printf("[AGENTE C %d] Solicita RESERVE: Job=%lu, Recurso=%s, Cantidad=%lu\n",
            ...
            if (datos != NULL) {
                ...
                } else if (estadoSolicitud == 0) {
                    printf("[AGENTE C %d] La reserva se agregó a la cola de espera\n", cliente->fd);
                } else {
        ...
        if (sscanf(mensaje, "RELEASE %lu %15s %lu", &jobId, recurso, &cant) == 3) {
            printf("[AGENTE C %d] Notifica RELEASE: Job=%lu, Recurso=%s, Cantidad=%lu\n",
                   cliente->fd, jobId, recurso, cant);
            ListaPromovidos lista = liberar_recurso(recNodo, tablaJobs, jobId,
                                                    tipo_recurso_desde_string(recurso));
...
void liberar_job(TablaJobs tablaJobs, unsigned long jobId, int epollFd, RecursosNodo recNodo) {
    ...
    while (actual != NULL) {
        ClienteConexion *cl = (ClienteConexion *)actual->datos->datosCliente;
        if (cl != NULL && cl->tipo != CLIENTE_ERLANG) {
            // ─── Recurso remoto: enviar RELEASE y cerrar conexión ───
            char rec[4];
            int cant = 0;
            ...
            enviar_formateado(cl->fd, "RELEASE %lu %s %d\n", jobId, rec, cant);
            epoll_ctl(epollFd, EPOLL_CTL_DEL, cl->fd, NULL);
            close(cl->fd);
            free(cl);
        } else if (cl != NULL && cl->tipo == CLIENTE_ERLANG) {
            // ─── Recurso local: liberar sin tocar el socket de Erlang ───
            TipoRecurso rec;
            if (actual->datos->recReservados->cpu > 0)
                rec = CPU;
            else if (actual->datos->recReservados->mem > 0)
                ...
                rec = GPU;
            // liberar_recurso actualiza la tabla y notifica a trabajos encolados
            liberar_recurso(recNodo, tablaJobs, jobId, rec);
        }
        actual = actual->sig;
    }
}
    ...
    if (strncmp(mensaje, "JOB_REQUEST", 11) == 0) {
        ...
        if (sscanf(mensaje, "JOB_REQUEST %lu%n", &jobId, &offset) == 1) {
            ...
            while (sscanf(ptr, " @%15[^:]:%15[^:]:%d%n", ip, recurso, &cantidad,
                ...
                if (strcmp(ip, miIp) == 0) {
                    // ============ RECURSO LOCAL ============
                    registrar_solicitud_propia(tablaJobs, jobId, tipoRec, cantidad,
                                               ip, puertoTcp, cliente);
                    int resultado = reservar_recurso(recNodo, tablaJobs, jobId,
                                                     tipoRec, cantidad,
                                                     ip, puertoTcp, cliente);
                    if (resultado == 1) {
                        tablajobs_recurso_granted(tablaJobs, jobId, ip, puertoTcp, tipoRec);
                        if (tablajobs_job_granted(tablaJobs, jobId)) {
                            enviar_formateado(cliente->fd, "JOB_GRANTED %lu\n", jobId);
                            printf("[ERLANG %d] Recibio JOB_GRANTED (local)\n", cliente->fd);
                        }
                    } else if (resultado == 0) {
                        printf("[AGENTE C] Recurso local encolado para Job=%lu\n", jobId);
                    } else {
                        enviar_formateado(cliente->fd, "JOB_DENIED %lu\n", jobId);
                    }
                } else {
                    // ============ RECURSO REMOTO ============
                    DatosNodo *datos = tablanodos_buscar(tablaNodos, ip);
                    if (datos != NULL) {
                        int fdSalida = crear_socket_saliente_nobloqueante(ip, datos->puerto, miIp);
                        ClienteConexion *nueva_conexion = crear_cliente(fdSalida, CONEXION_SALIENTE, ip, NULL);
                        registrar_solicitud_propia(tablaJobs, jobId, tipoRec, cantidad,
                                                   ip, datos->puerto, nueva_conexion);
            ...
            printf("[ERLANG %d] Finalizado el parseo de dependencias para Job %lu\n",
                   cliente->fd, jobId);
        } else {
            fprintf(stderr, "[ERLANG %d] Error de sintaxis en el encabezado del JOB_REQUEST\n",
                    cliente->fd);
        }
        ...
        if (sscanf(mensaje, "JOB_RELEASE %lu", &jobId) == 1) {
            printf("[ERLANG %d] Planificador notifica fin de Job ID: %lu\n",
                   cliente->fd, jobId);
            liberar_job(tablaJobs, jobId, epollFd,recNodo);
        }
    }
}
...
void registrar_nodo(int udp_sock, TablaNodos tablaNodos) {
    ...
    int bytes_recibidos = recvfrom(udp_sock, buffer_udp, sizeof(buffer_udp) - 1, 0,
    ...
    if (bytes_recibidos > 0) {
        ...
        if (strncmp(buffer_udp, "ANNOUNCE", 8) == 0) {
            ...
            if (!datos) return;
            if (inet_ntop(AF_INET, &(sender_addr.sin_addr), datos->ip, INET_ADDRSTRLEN) == NULL) {
                perror("inet_ntop fallo al extraer la IP");
                free(datos);
                return;
            }
            if (sscanf(buffer_udp, "ANNOUNCE %hu %63[^\n]", &datos->puerto, datos->recursos) == 2) {
                printf("Descubierto nodo activo: IP=%s, Puerto=%hu, Recursos=%s\n",
...
void anuncio_broadcast(int puerto_udp, int puertoTcpEscucha, const char *miIp,
    ...
    setsockopt(send_sock, SOL_SOCKET, SO_BROADCAST, &broadcast_enable, sizeof(broadcast_enable));
